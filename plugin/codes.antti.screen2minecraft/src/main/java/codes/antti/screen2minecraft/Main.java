			...
			public void run() {
				try {
					...
					for (int k = 0; k < blockDatas.size(); k++) {
						...
						blocks.put(blockData.get("game_id_13").getAsString().split(":")[1].toUpperCase(),
						...
						if (index != bufferSize - 1) {
							...
							for (int y = 0; y < screenHeight / downScale; y++) {
								for (int x = 0; x < screenWidth / downScale; x++) {
									...
									int dataIndex = x * bytesPerPixel * downScale
											+ y * screenWidth * bytesPerPixel * downScale;
									Material block = Material.valueOf(
											findClosest(data[dataIndex + 2], data[dataIndex + 1], data[dataIndex]));
